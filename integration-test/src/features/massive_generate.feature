Feature: Massive Generation
  ...
  Scenario: MT_01_MultipleTemplates_COMPLETED - Generazione Massiva con successo su multipli template
  ...
  Scenario: MT_01_MultipleTemplates_COMPLETED_WITH_FAILURES - Generazione Massiva con successo parziale su multipli template
    Given the creditor institution in the storage:
      | variableName       | value                                  |
      | taxCode            | "80034390585"                          |
      | fullName           | "Comune di Test"                       |
    ...
    And the response should be in JSON format
    And the response should contain the folderId
    And the request is in status COMPLETED_WITH_FAILURES after 10000 ms
    And download url is recoverable with the folderId
    And can download content using signedUrl
